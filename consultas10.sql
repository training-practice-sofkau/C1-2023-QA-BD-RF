...
SELECT YEAR(s.fecha) AS year, MONTH(s.fecha) AS month, COUNT(s.ID) AS cantidad_servicios
FROM servicios s
...
SELECT e.nombre, COUNT(s.ID) AS cantidad_servicios, SUM(v.liga_ganancia) AS total_ganado
FROM empleados e
LEFT JOIN servicios s ON e.ID = s.empleado_id
LEFT JOIN ventas v ON e.ID = v.empleado_id
